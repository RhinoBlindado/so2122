...
int main(int argc, char *argv[])
{
    ...
    int cause_cat[2], cause_cut[2];
    pid_t pid;

    umask(0);
    pipe(cause_cat); // Llamada al sistema para crear un cauce sin nombre
    ...
    {
        close(cause_cat[0]);
        dup2(cause_cat[1],STDOUT_FILENO);
    ...
    else
    {
        ...
        if (pid == 0) 
        {
            dup2(cause_cat[0],STDIN_FILENO);
            dup2(cause_cut[1],STDOUT_FILENO);
        ...
        { 
            ...
            while (numBytes = read(cause_cut[0], buffer, MAX_SIZE))
            {
                buffer[MAX_SIZE] = '\0';
            ...
            if( (fileDecrp = open("./resultado_titanic2.txt", O_CREAT | O_WRONLY | O_TRUNC, 0660)) < 0)
            {
