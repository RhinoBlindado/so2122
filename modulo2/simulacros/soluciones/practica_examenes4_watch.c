...
int main(int argc, char *argv[])
{
    ...
    if(PID == 0){
        ...
        execlp("./practica_examenes4_explorar", "practica_examenes4_explorar", argv[1], argv[2], NULL);
        ...
        if(PID == 0){
            ...
            dup2(fd_cut[0], STDIN_FILENO); //el resultado de cat titanic.csv lo redirigimos a la entrada de cut
            dup2(fd[1], STDOUT_FILENO); // vamos a escribir el resultado de cat titanic.csv | cut -d";" -f 6 en la salida estandar

            while (read(fd[0], buf, 1)) {
//            printf("%s", buf);
                sprintf(filename_aux, "%s%s",filename_aux, buf);
                if(!strcmp(buf, "\n")){ //tenemos el nombre del fichero completo
//                printf("%s", filename_aux);

                    const char* filen = (char*) malloc(strlen(filename_aux)* sizeof(char)+1);
                    sprintf(filen, "%s", filename_aux);
//                printf("Filename: %s", filen);
                    strcpy(filename_aux, "");

                    execlp("cut","cut" ,"-f", "1", "-d", ",", filen, NULL);
